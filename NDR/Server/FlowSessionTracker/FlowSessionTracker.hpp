                    ...
                    */
                    public:
                        FlowSessionRule(json event) : Event(event)
                        {
                            ...
                            this->stage_index = event["body"]["session_rule"]["stage_index"].get<unsigned long long>();
                        ...
                        unsigned long long stage_index;
                    ...
                    EventLoop = std::thread(
                        [this]()
                        {
                            while (this->is_running)
                            {
                                ...
                                //FlowEvent::Event evt(Message.message);
                                ...
                                {
                                    evt = std::make_shared<FlowEvent::FlowSessionStart>(Message.message["body"]["session_start"]); 
                                }
                                else if(Message.message["body"].contains("session_timeout"))
                                {
                                    evt = std::make_shared<FlowEvent::FlowSessionTimeout>(Message.message["body"]["session_timeout"]); 
                                }
                                else if(Message.message["body"].contains("session_rule"))
                                {
                                    evt = std::make_shared<FlowEvent::FlowSessionRule>(Message.message["body"]["session_rule"]); 
                                }
                                else
                                    continue;
                    ...
                    if( dynamic_cast< FlowEvent::FlowSessionStart* >(evt.get()) )
                    {
                        ...
                        Node_Mapped[evt->FlowSessionId] = SessionNode::FlowSessionNode{
                            .header = {
                                ...
                                .seen = {
                                    .first_seen = evt->NanoTimestamp,
                                    .last_seen = evt->NanoTimestamp
                                }
                            }
                        };
