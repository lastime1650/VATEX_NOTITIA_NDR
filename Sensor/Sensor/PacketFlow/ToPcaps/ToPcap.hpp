        ...
        namespace ToPcap
        {
            ...
            class ToPcap
            {
                ...
                public:
                    ...
                    bool AppendPacket( unsigned long long timestamp, const uint8_t* RealPacketDataPtr, const size_t PacketDataLen, bool with_mutex = false )
                    {
                        std::shared_ptr<std::mutex> lock_mtx;
                        if (with_mutex && mtx)
                            lock_mtx = mtx;  // shared_ptr로 복사
                        ...
                        if(PacketDataLen || RealPacketDataPtr)
                        {
                            ...
                            */
                            pcap_pkthdr pkthdr;
                            pkthdr.ts_sec   = timestamp / 1000000000ULL;
                            pkthdr.ts_usec  = (timestamp % 1000000000ULL) / 1000;
                            pkthdr.incl_len = PacketDataLen;
                            pkthdr.orig_len = PacketDataLen;
                            this->FileHandlerInstance.writeToFile(
                                this->PcapFilePath,
                            ...
                            /*
                                2. 패킷 바이너리 이어서.
                            */
                            const uint8_t* PacketData = RealPacketDataPtr;
                            const uint8_t* PacketDataEnd = PacketData + PacketDataLen;
